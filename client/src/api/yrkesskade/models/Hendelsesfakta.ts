...
export type Hendelsesfakta = {
    ...
    aarsakUlykkeTabellAogE: Array<'Støt/treff av gjenstand' | 'Sammenstøt, bitt, spark, osv. (dyr eller menneske)' | 'Sammenstøt med gjenstand eller påkjørsel' | 'Velt' | 'Klemt/fanget' | 'Fall av person' | 'Stukket/kuttet av skarp/spiss gjenstand' | 'Kontakt med elektrisk spenning, farlige stoffer' | 'Høy temperatur' | 'Lav temperatur' | 'Kjemikalier' | 'Eksplosjon, sprenging, brann' | 'Trusler om vold' | 'Påført voldsskade' | 'Druknet, begravd, omsvøpt' | 'Fysisk eller psykisk belastning' | 'Alternativene passer ikke'>;
